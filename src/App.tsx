...
import { BrainCircuit, GraduationCap, Loader2, Scale, X, ArrowDown, ArrowUp, ArrowRight, User as UserIcon } from 'lucide-react';
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    if (user && token) {
      fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/user/favorites`, {
  ...
  const toggleSave = async (college: College) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/user/favorites`, {
        method: 'POST',
        headers: { 
    ...
    <div className="min-h-screen overflow-hidden text-slate-950 font-sans selection:bg-[#f4a261]/30">
          ...
          <div className="flex min-h-20 flex-col gap-4 py-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-full border border-white/80 bg-white/65 p-1 text-sm shadow-sm shadow-slate-200/60">
              ...
              {user ? (
                ...
                <button onClick={() => setIsAuthModalOpen(true)} className="relative rounded-full px-4 py-2 text-sm font-semibold transition-all text-slate-600 hover:bg-white hover:text-[#203d1f]">
                  Sign In
                </button>
              )}
            </div>
      ...
      {compareList.length > 0 && (
        <motion.div 
          ...
          className="fixed bottom-6 z-50 w-[min(92vw,360px)] cursor-grab active:cursor-grabbing"
          initial={{ opacity: 0, y: 60, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ type: 'spring', stiffness: 300, damping: 25 }}
        >
          <div className="surface overflow-hidden rounded-3xl p-4 shadow-2xl border border-slate-200/60">
